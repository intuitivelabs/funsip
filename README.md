...
| `playAudio(filename, {ringingTime})` | Answer the incoming INVITE locally by streaming a WAV file as G.711 µ-law RTP at 8 kHz. The proxy becomes the UAS for this call. Flow: 180 Ringing immediately → wait `ringingTime` seconds (default 0) → 200 OK with funsip's own SDP (PT 0, PCMU) → start sending 20 ms RTP frames to the caller's SDP-advertised RTP address. The audio file may be 16-bit PCM, µ-law or A-law mono at 8 kHz; non-µ-law formats are converted on the fly. The file loops on EOF. The proxy answers BYE locally (no forwarding) and emits `call-start` and `call-end` events carrying `event.filename`. Use after `setupDialog({})` so the filename also makes it into the timeout-driven call-end event. Halts the script — no `proxy()` / `sendResponse()` after it. |
| `limit(key, {window, maxCount, ttl, blacklist})` | Sliding-window rate limit. Counts how many requests have matched `key` in the last `window` seconds. If the count would exceed `maxCount`, the request is answered `403 Forbidden`, the script is halted (no further script code runs and the server does not try to forward), and the key is added to a "deny-until" table for `ttl` seconds — every subsequent request for the same key is denied without re-counting. With `blacklist:true` (default false), the source IP is added to a separate "deny-until" table for the same `ttl`, so any request from that IP is denied regardless of key. Per-key memory is bounded at `maxCount` timestamps; once that many in-window hits exist, the next hit denies without growing the structure. Example: `if (req.ip !== "192.168.0.64") limit(req.ip + '#' + req.from.uri, {window: 3600, maxCount: 5, ttl: 7200, blacklist: true});` |
